...
We also performed a small benchmark for single-point imaginary-time tests. Considering measurement cost and accuracy, GRDM-QMC outperforms the BRA method by 85% to 94%. Even so, the BRA method may have an advantage over GRDM in computing off-diagonal quantities in domain-wall regions, such as disorder operators, whereas GRDM is constrained by the subsystem size defined by the RDM. 
